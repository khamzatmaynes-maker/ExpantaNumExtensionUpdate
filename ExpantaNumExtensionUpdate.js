...
class ExpantaNumExtension { // правильное название класса
  getInfo() {
    return {
      id: 'expantanum',
      name: 'ExpantaNum',
      color1: '#8B00FF', // фиолетовый
      color2: '#4B0082',
      blocks: [
        {
          opcode: 'expantaTrue',
        ...
        {
          opcode: 'expantaAdd', // имя функции ниже
          blockType: Scratch.BlockType.REPORTER,
          text: '[a] + [b]', // блок выглядит как "10 + 100"
          arguments: {
            a: {
              type: Scratch.ArgumentType.STRING, // используем STRING, чтобы принимать большие числа как текст
              defaultValue: '10'
            },
            b: {
...
, {
  ...
  arguments: {
    a: {
      type: Scratch.ArgumentType.BOOLEAN, // используем STRING, чтобы принимать большие числа как текст
    },
    b: {
      type: Scratch.ArgumentType.BOOLEAN, // используем STRING, чтобы принимать большие числа как текст
    },
...
,{
  opcode: 'expantaSetVar',
  blockType: Scratch.BlockType.COMMAND,
  text: 'Set [var] value [val]',
  arguments: {
    var: {
      type: Scratch.ArgumentType.STRING,
      menu: 'variablesMenu'
...
, {
  opcode: 'expantaSetVarHyperE',
  blockType: Scratch.BlockType.COMMAND,
  text: 'Set [var] value [val] .toHyperE',
  arguments: {
    var: {
      type: Scratch.ArgumentType.STRING,
      menu: 'variablesMenu'
...
,{
  opcode: 'expantaIncreaseVar',
  blockType: Scratch.BlockType.COMMAND,
  text: 'Increase [var] by [val]',
  arguments: {
    var: {
      type: Scratch.ArgumentType.STRING,
      menu: 'variablesMenu'
...
, {
  opcode: 'expantaIncreaseVarHyperE',
  blockType: Scratch.BlockType.COMMAND,
  text: 'Increase [var] by [val] .toHyperE',
  arguments: {
    var: {
      type: Scratch.ArgumentType.STRING,
      menu: 'variablesMenu'
  ...
  getVariablesList() {
    try {
      const rt = typeof Scratch !== 'undefined' && Scratch.vm && Scratch.vm.runtime;
      if (!rt || !rt.targets || !rt.targets.length) return [{ text: '-', value: '-' }];
      const vars = [];
      for (const t of rt.targets) {
        for (const v of Object.values(t.variables)) {
          if (v.type === '') vars.push({ text: v.name, value: v.name });
        }
      }
  ...
  // функция, которая выполняется при использовании блока
  expantaTrue() { return true; }
  expantaFalse() { return false; }
  expantaZero(args) { return new ExpantaNum(String(args.x) || '0').toString(); }
  expantaAdd(args) {
    try {
      // Теперь используем настоящую библиотеку ExpantaNum
      let numA = new ExpantaNum(String(args.a) || '0');
      let numB = new ExpantaNum(String(args.b) || '0');
      let result = numA.add(numB);
      return result.toString(); // вернёт полную строку числа
      // или return result.toScientific(6); // красивая форма 1.234567e+100
    } catch (e) {
      return 'Error: ' + (e.message || 'проверь ввод');
    }
  }
  ...
  expantaPersent(args) {
    try {
      ...
      let numA = new ExpantaNum(String(args.a) || '0');
      let numB = new ExpantaNum(String(args.b) || '0');
      let result = numA.div(100).mul(numB)
      return result.toString(); // вернёт полную строку числа
      // или return result.toScientific(6); // красивая форма 1.234567e+100
    } catch (e) {
      return 'Error: ' + (e.message || 'проверь ввод');
  ...
  expantaArrow(args) {
    try {
      ...
      let numA = new ExpantaNum(String(args.a) || '0');
      let numB = new ExpantaNum(String(args.b) || '0');
      let numC = new ExpantaNum(String(args.c) || '0');
      let result = ExpantaNum.arrow(numA, numB, numC)
      return result.toString(); // вернёт полную строку числа
      // или return result.toScientific(6); // красивая форма 1.234567e+100
    } catch (e) {
      return 'Error: ' + (e.message || 'проверь ввод');
...
expantaToHyperE(args) {
  try {
    let num = new ExpantaNum(String(args.a) || '0');
    return ExpantaNum(num).toHyperE();
  } catch (e) {
    return 'Error';
  }
}
expantaToString(args) {
  try {
    return new ExpantaNum(String(args.a) || '0').toString();
  } catch (e) {
    return 'Error';
  }
}
expantaSetVar(args) {
  try {
    ...
    if (!rt) return;
    for (const t of rt.targets) {
      const v = Object.values(t.variables).find(v => v.name === args.var);
      if (v) {
        v.value = new ExpantaNum(String(args.val) || '0').toString();
        return;
      }
    }
  } catch(e) {}
}
expantaSetVarHyperE(args) {
  try {
    const rt = Scratch.vm && Scratch.vm.runtime;
    if (!rt) return;
    for (const t of rt.targets) {
      const v = Object.values(t.variables).find(v => v.name === args.var);
      if (v) {
        v.value = new ExpantaNum(String(args.val) || '0').toHyperE();
        return;
      }
    }
  } catch (e) {}
}
expantaIncreaseVar(args) {
  try {
    const rt = Scratch.vm && Scratch.vm.runtime;
    if (!rt) return;
    for (const t of rt.targets) {
      const v = Object.values(t.variables).find(v => v.name === args.var);
      if (v) {
        let current = new ExpantaNum(String(v.value) || '0');
        let delta = new ExpantaNum(String(args.val) || '0');
        v.value = current.add(delta).toString();
        return;
      }
    }
  } catch(e) {}
}
expantaIncreaseVarHyperE(args) {
  try {
    const rt = Scratch.vm && Scratch.vm.runtime;
    if (!rt) return;
    for (const t of rt.targets) {
      ...
      if (v) {
        let current = new ExpantaNum(String(v.value) || '0');
        let delta = new ExpantaNum(String(args.val) || '0');
        v.value = current.add(delta).toHyperE();
        return;
      }
    }
  } catch (e) {}
...
// Обязательно зарегистрировать расширение!
Scratch.extensions.register(new ExpantaNumExtension());